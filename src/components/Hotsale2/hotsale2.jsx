...
import { fetchHotsale2 } from "../Redux/Hotsale2/ActionHotsale2";
...
const Hotsale2 = () => {
  const { loading, error, hotsale2 } = useSelector((state) => state.hotsale2);
  ...
  useEffect(() => {
    dispatch(fetchHotsale2());
  }, []);
  if (loading) return <h1>Loading...</h1>;
  if (error) return <h1>Error: {error}</h1>;
  return (
    <div className="hotsale2 w-[100%] flex flex-row flex-wrap items-center overflow-hidden lg:overflow-visible p-[20px]">
      <div className="hotsale2-center w-[1345px] lg:border-[0.1px] mx-auto lg:border-solid lg:border-[lightgray] rounded-[10px] box-border lg:shadow-sm flex  flex-col items-center">
        <div className="hotsale2-center-top w-full h-[20%] font-[iranyekanmedium] pt-[20px] text-[#2b2b2b]  flex items-center justify-between lg:justify-center px-2 lg:px-5 relative">
          <div className="flex gap-2 justify-center items-center">
            <img src={hotsale2.fire} alt="" className="w-[24px] h-[24px]"/>
            <div className="text-[17px]">{hotsale2.title}</div>
            </div>
         </div>
                ...
                {hotsale2.category?.map((item) => {
                    ...
                    <div className="swiper-slide-parent !w-[100%] h-[100%] my-0 mx-auto flex flex-col flex-wrap items-start gap-x-[5px] pb-[10px]">
                      {item.section?.map((item) => {
                        return (
                          <div key={item.id}>
                            <div onClick={()=>handleAdd(item,"hotsale2")} className="swiper-slide-child w-[100%] h-[31%] flex flex-row flex-wrap items-center gap-y-[5px] py-0 px-[15px]" >
